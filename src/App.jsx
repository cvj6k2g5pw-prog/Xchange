...
const T = {
  de: {
    ...
    onboardingTitle: "Profil erstellen", step: "Schritt",
  ...
  en: {
    ...
    onboardingTitle: "Create Profile", step: "Step",
...
export default function SimpleXchange() {
  ...
  // Card photo index for multi-photo swiping
  ...
  async function checkProfileAndRoute(userId) {
    ...
    if (data) {
      ...
      // If profile incomplete, show onboarding
  ...
  async function loadProfiles(userId, appliedFilter) {
    ...
    let result = data || [];
    if (f.language) {
      result = result.filter(p => (p.languages || []).some(l => l.toLowerCase().includes(f.language.toLowerCase())));
    }
    setProfiles(result);
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        ...
        if ((msg.sender_id === session.user.id && msg.receiver_id === activeChat.id) ||
          ...
          if (msg.sender_id === activeChat.id) {
            supabase.from("messages").update({ seen: true }).eq("id", msg.id);
          }
        }
      })
  ...
  async function markMessagesAsSeen() {
    ...
    await supabase.from("messages").update({ seen: true })
      .eq("sender_id", activeChat.id).eq("receiver_id", session.user.id).eq("seen", false);
  }
  ...
  function resetFilter() {
    ...
    setFilter(empty); setActiveFilter(empty);
    setShowFilter(false);
      ...
      <div style={{ background: DARK, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Courier New', monospace", padding: 24 }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>
            {steps.map((_, i) => (
              <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= onboardingStep ? NEON : BORDER }} />
            ))}
          </div>
            ...
            {step.fields.map(field => (
              <div key={field}>
                ...
                {field === "bio"
                  ...
                  : <input type={field === "age" ? "number" : "text"} value={onboardingForm[field]} onChange={e => setOnboardingForm({...onboardingForm, [field]: e.target.value})} placeholder={t[field]} style={inputStyle} />
                }
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
            {onboardingStep > 0 && (
              <button onClick={() => setOnboardingStep(s => s - 1)} style={{ background: "transparent", color: "#555", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "14px 20px", cursor: "pointer", fontFamily: "inherit", fontSize: 14 }}>←</button>
            )}
          ...
          {!isLast && (
            <div onClick={saveOnboarding} style={{ color: "#444", fontSize: 12, textAlign: "center", marginTop: 12, cursor: "pointer" }}>{t.skip}</div>
          )}
        </div>
      </div>
    );
        ...
        {activeTab === "swipe" && (
          <div style={{ padding: "16px 16px 0", display: "flex", flexDirection: "column", alignItems: "center", height: "calc(100vh - 120px)" }}>
            ...
            <div style={{ width: "100%", maxWidth: 360, display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
              <button onClick={() => setShowFilter(true)} style={{ background: hasActiveFilter ? NEON : SURFACE, color: hasActiveFilter ? DARK : "#666", border: `1px solid ${hasActiveFilter ? NEON : BORDER}`, borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div style={{ position: "relative", height: 300 }}>
                      ...
                      {[current.photo_url, current.photo_url_2].filter(Boolean).length > 1 && (
                        <div style={{ position: "absolute", top: 10, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 6 }}>
                          {[current.photo_url, current.photo_url_2].filter(Boolean).map((_, i) => (
                            <div key={i} onClick={(e) => { e.stopPropagation(); setCardPhotoIndex(i); }}
                              style={{ width: 8, height: 8, borderRadius: "50%", background: i === cardPhotoIndex ? "#fff" : "#ffffff55", cursor: "pointer" }} />
                          ))}
                        </div>
                      )}
                      ...
                      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, #000000ee)", padding: "36px 20px 18px" }}>
                        <div style={{ color: "#fff", fontWeight: 900, fontSize: 22, display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          {current.verified && <span style={{ color: BLUE, fontSize: 18 }} title={t.verified}>✓</span>}
                        </div>
                ...
                <div style={{ padding: "12px 16px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                  ...
                  <div>
                    <div style={{ color: "#fff", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", gap: 6 }}>
                      {activeChat.name}
                      {activeChat.verified && <span style={{ color: BLUE, fontSize: 14 }}>✓</span>}
                    </div>
                        ...
                        <div style={{ maxWidth: "72%" }}>
                          ...
                          {isMe && (
                            <div style={{ fontSize: 10, color: "#555", marginTop: 2, textAlign: "right", display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 4 }}>
                              ...
                              <span style={{ color: msg.seen ? BLUE : "#555", fontSize: 12 }}>{msg.seen ? "✓✓" : "✓"}</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 12 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <button onClick={() => fileInputRef.current?.click()} style={{ position: "absolute", bottom: 0, right: 0, width: 26, height: 26, borderRadius: "50%", background: NEON, border: "none", cursor: "pointer", fontSize: 12, boxShadow: `0 0 10px ${NEON}66` }}>📷</button>
                ...
                <div style={{ position: "relative" }}>
                  <div style={{ width: 80, height: 80, borderRadius: "50%", border: `2px dashed ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden" }} onClick={() => fileInput2Ref.current?.click()}>
                    {profile?.photo_url_2
                      ? <img src={profile.photo_url_2} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              ...
              <div style={{ color: "#fff", fontWeight: 900, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                {profile?.name || "—"}
                {profile?.verified && <span style={{ color: BLUE, fontSize: 18 }} title={t.verified}>✓</span>}
              </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#000000bb", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxWidth: 480 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={filter.country} onChange={e => setFilter({...filter, country: e.target.value})} placeholder={t.allCountries} style={inputStyle} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ color: "#555", fontSize: 10, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>{t.filterAge}</div>
              <div style={{ display: "flex", gap: 10 }}>
                <input type="number" value={filter.ageMin} onChange={e => setFilter({...filter, ageMin: e.target.value})} placeholder={t.from + " 14"} style={{ ...inputStyle, flex: 1 }} />
                <input type="number" value={filter.ageMax} onChange={e => setFilter({...filter, ageMax: e.target.value})} placeholder={t.to + " 19"} style={{ ...inputStyle, flex: 1 }} />
              </div>
            </div>
      ...
      {matchPopup && (
        <div style={{ position: "fixed", inset: 0, background: "#000000cc", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: CARD_BG, border: `1px solid ${NEON}`, borderRadius: 20, padding: 32, textAlign: "center", maxWidth: 300, boxShadow: `0 0 60px ${NEON}44` }}>
            ...
            <div style={{ color: "#fff", fontWeight: 700, marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              {matchPopup.name}
              {matchPopup.verified && <span style={{ color: BLUE }}>✓</span>}
            </div>
    ...
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, marginBottom: 10, cursor: "pointer" }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ color: "#fff", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
          {m.name}
          {m.verified && <span style={{ color: "#1DA1F2", fontSize: 14 }}>✓</span>}
        </div>
