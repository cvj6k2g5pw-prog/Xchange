...
const T = {
  de: {
    ...
    sendMsg: "Nachricht...", send: "Senden", logout: "Abmelden",
  ...
  en: {
    ...
    sendMsg: "Message...", send: "Send", logout: "Log Out",
...
export default function SimpleXchange() {
  ...
  async function handleAuth() {
    ...
    if (authMode === "login") {
      ...
      if (error) { setAuthError(error.message); }
        ...
        await supabase.from("profiles").insert({ id: data.user.id, name: "", age: null, country: "", wanted_countries: [], languages: [], hobbies: [], bio: "" });
      }
    }
  ...
  async function saveProfile() {
    ...
    if (!error) { setProfile(updates); setProfileMsg(t.profileSaved); }
        ...
        {activeTab === "profile" && (
          <div style={{ padding: 20, overflowY: "auto", maxHeight: "calc(100vh - 120px)" }}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <Avatar p={profile || {}} size={72} large />
              <div style={{ color: "#fff", fontWeight: 900, fontSize: 18, marginTop: 10 }}>{profile?.name || "—"}</div>
